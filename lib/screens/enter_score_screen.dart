...
class EnterScoreScreenState extends State<EnterScoreScreen> {
  ...
  String?
  selectedPersonalFirearmId; // ID of selected personal firearm from FirearmEntry
  bool get _showManualFirearmFields =>
      selectedPersonalFirearmId == null ||
      selectedPersonalFirearmId == 'manual';
  ...
  Future<void> _initializeScreen() async {
    ...
    if (widget.scoringMode) {
      setState(() {
        ...
        if (widget.initialPractice != null &&
            widget.initialPractice!.isNotEmpty) {
          selectedPractice = widget.initialPractice;
        }
        if (widget.initialCaliber != null &&
            widget.initialCaliber!.isNotEmpty) {
          selectedCaliber = widget.initialCaliber;
        }
        if (widget.initialFirearmId != null &&
            widget.initialFirearmId!.isNotEmpty) {
          selectedFirearmId = widget.initialFirearmId;
        }
        if (widget.initialFirearm != null &&
            widget.initialFirearm!.isNotEmpty) {
  ...
  void _populateEditFields() {
    ...
    setState(() {
      ...
      targetImage = entry.targetFilePath != null
          ? File(entry.targetFilePath!)
          : null;
      thumbnailImage = entry.thumbnailFilePath != null
  ...
  Future<void> _loadLastSelections() async {
    ...
    setState(() {
      ...
      final lastPractice =
          (widget.editEntry?.practice != null &&
      ...
      final lastFirearmId =
          (widget.editEntry?.firearmId != null &&
  ...
  void _confirmSaveEntry() async {
    ...
    if (isEventScoring) {
      ...
      final hasMissingTargetScores = _targetScores
          .take(expectedTargets)
          .any((score) => score == null);
      if (hasMissingTargetScores) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please enter a score for each target.'),
          ),
        );
        return;
      }
    ...
    final roundsCounterEnabled = Provider.of<RoundsCounterProvider>(
      context,
      listen: false,
    ).enabled;
    ...
    int? roundsUsed;
    if (isEventScoring) {
      final totalRoundsUsed = _targetBreakdowns.fold<int>(
        0,
        (sum, breakdown) => sum + ScoreCalculatorUtils.roundsCounted(breakdown),
      );
      roundsUsed = totalRoundsUsed > 0 ? totalRoundsUsed : null;
    } else if (_scoreBreakdown != null) {
      roundsUsed = ScoreCalculatorUtils.roundsCounted(_scoreBreakdown);
    }
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  final targetXTotal = isEventScoring
                      ? _targetXCounts.whereType<int>().fold<int>(
                          0,
                          (sum, x) => sum + x,
                        )
                  ...
                  final eventTargetCount =
                      _getRequiredTargetCountForSelectedEvent() ?? 1;
                  ...
                  final newEntry = ScoreEntry(
                    ...
                    thumbnailFilePath: isEventScoring
                        ? null
                        : thumbnailImage?.path,
                    targetCaptured: isEventScoring
                        ? _targetImages.any((f) => f != null)
                        : targetImage != null,
                    x: targetXTotal,
                    ...
                    thumbnailFilePaths: isEventScoring
                        ? List<String>.generate(
                            eventTargetCount,
                            (i) => _targetThumbnails[i]?.path ?? '',
                          )
                        : (thumbnailImage?.path != null
                              ? [thumbnailImage!.path]
                              : null),
                    targetsCaptured: isEventScoring
                        ? List<bool>.generate(
                            eventTargetCount,
                            (i) => _targetImages[i] != null,
                          )
                        : [targetImage != null],
                    xs: isEventScoring
                        ? List<int>.generate(
                            eventTargetCount,
                            (i) => _targetXCounts[i] ?? 0,
                          )
                        : (xController.text.isNotEmpty
                    ...
                    score10s: isEventScoring
                        ? List<int>.generate(
                            eventTargetCount,
                            (i) => _targetBreakdowns[i]?[10] ?? 0,
                          )
                        : (_scoreBreakdown != null
                              ? [_scoreBreakdown![10] ?? 0]
                              : null),
                    score9s: isEventScoring
                        ? List<int>.generate(
                            eventTargetCount,
                            (i) => _targetBreakdowns[i]?[9] ?? 0,
                          )
                        : (_scoreBreakdown != null
                              ? [_scoreBreakdown![9] ?? 0]
                              : null),
                  ...
                  if (roundsCounterEnabled &&
                    ...
                    final sightersCount =
                        int.tryParse(sightersController.text) ?? 0;
                    if (sightersCount > 0) {
  ...
  bool _isFirearmEligibleForPractice(
    String practiceOrEvent,
    String firearmCode,
  ) {
  ...
  List<int>? _getPerTargetRoundsForSelectedEvent() {
    return ScoreCalculatorUtils.getPerTargetRounds(
      eventName: selectedPractice,
  ...
  int? _getMaxScoreForSelectedEvent() {
    return ScoreCalculatorUtils.getMaxScore(
      eventName: selectedPractice,
      firearmCode: selectedFirearmId,
      requireFirearm: true,
    );
                                  ...
                                  if (v != null) {
                                    ...
                                    setState(() {
                                      ...
                                      if (selectedFirearmId != null &&
                                          selectedFirearmId!.isNotEmpty) {
                                        if (!_isFirearmEligibleForPractice(
                                          v,
                                          selectedFirearmId!,
                                        )) {
                                  ...
                                  await showPracticeSelectionDialog(
                                    ...
                                    onSelectionChanged: () {
                                      setState(() {
                                        ...
                                        final currentPractices =
                                            DropdownValues.practices;
                                        if (!currentPractices.contains(
                                          selectedPractice,
                                        )) {
                                          selectedPractice =
                                              currentPractices.first;
                                        }
                                      });
                                    },
                                  );
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ValueListenableBuilder(
                          valueListenable: Hive.box<FirearmEntry>(
                            'firearms',
                          ).listenable(),
                          builder: (context, Box<FirearmEntry> box, _) {
                            ...
                            return DropdownButtonFormField<String>(
                              ...
                              onChanged: (v) {
                                setState(() {
                                  selectedPersonalFirearmId = v == 'manual'
                                      ? null
                                      : v;

                                  if (v != null && v != 'manual') {
                                    // Auto-populate from personal firearm
                                    final selectedFirearm = personalFirearms
                                        .firstWhere(
                                          (f) => f.id == v,
                                    ...
                                    if (selectedFirearm.caliber.isNotEmpty) {
                                      final calibersList =
                                          DropdownValues.calibers;
                                      if (calibersList.contains(
                                    ...
                                    if (selectedFirearm.myFirearmID != null &&
                                        selectedFirearm
                                            .myFirearmID!
                                            .isNotEmpty) {
                                      final firearmIdsList =
                                          DropdownValues.firearmIds;
                                      if (firearmIdsList.contains(
                                        selectedFirearm.myFirearmID,
                                      )) {
                                        selectedFirearmId =
                                            selectedFirearm.myFirearmID;
                                      }
                                    ...
                                    if (selectedFirearm.nickname?.isNotEmpty ==
                                        true) {
                                      firearmController.text =
                                          selectedFirearm.nickname!;
                                    }
                                  }
                                });
                              },
                                    ...
                                    child: IconButton(
                                      ...
                                      onPressed: () async {
                                        await showCaliberSelectionDialog(
                                          ...
                                          onSelectionChanged: () {
                                            setState(() {
                                              final currentCalibers =
                                                  DropdownValues.calibers;
                                              if (!currentCalibers.contains(
                                                selectedCaliber,
                                              )) {
                                                selectedCaliber =
                                                    currentCalibers.first;
                                              }
                                            });
                                          },
                                        );
                                            ...
                                            if (!_isFirearmEligibleForPractice(
                                              ...
                                              await showDialog(
                                                ...
                                                builder: (context) => AlertDialog(
                                                  title: const Text(
                                                    'Firearm Not Eligible',
                                                  ),
                                                  content: const Text(
                                                    'That firearm is not eligible for this event.',
                                                  ),
                                                  actions: [
                                                    TextButton(
                                                      onPressed: () =>
                                    ...
                                    child: IconButton(
                                      ...
                                      onPressed: () async {
                                        await showFirearmSelectionDialog(
                                          ...
                                          onSelectionChanged: () {
                                            setState(() {
                                              final currentFirearmIds =
                                                  DropdownValues.firearmIds;
                                              if (!currentFirearmIds.contains(
                                                selectedFirearmId,
                                              )) {
                                                selectedFirearmId =
                                                    currentFirearmIds.first;
                                              }
                                            });
                                          },
                                        );
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (widget.eventScoringMode) ...[
                          Builder(
                            builder: (_) {
                              final eventTotal = _targetScores
                                  .whereType<int>()
                                  .fold<int>(0, (sum, s) => sum + s);
                              final eventXTotal = _targetXCounts
                                  .whereType<int>()
                                    ...
                                    children: [
                                      Expanded(
                                        child: _buildGradientButton(
                                          label: "Score\nCalculator",
                                          onPressed: () =>
                                              _openScoreCalculatorForTarget(i),
                                          primaryColor: primaryColor,
                            ...
                            children: [
                              Expanded(
                                child: _buildGradientButton(
                                  label: "Score\nCalculator",
                                  onPressed: () async {
                                    final calcTotalRounds =
                                        _getTotalRoundsForSelectedEvent();
                                    final result =
                                        await showScoreCalculatorDialog(
                                          context: context,
                                          totalRounds: calcTotalRounds,
                                          selectedPractice: selectedPractice,
                                          selectedFirearmId: selectedFirearmId,
                              ...
                              Expanded(
                                child: _buildGradientButton(
                                  label: "Basic\nScore",
                                  onPressed: () => _showBasicScoreDialog(
                                    context,
                                    primaryColor,
                                  ),
                                  primaryColor: Colors.orange,
    ...
    try {
      ...
      final eligibleFirearms = eligibleIds.map((id) {
        return DropdownValues.masterFirearmTable.firstWhere(
          (f) => f.id == id,
          orElse: () =>
              FirearmInfo(id: id, code: 'Unknown ID: $id', gunType: ''),
        );
      }).toList();
  ...
  List<Widget> _buildPreNotesCard(
  ...
  Future<void> _showBasicScoreDialog(
    BuildContext context,
    Color primaryColor,
  ) async {
...
class _BasicScoreDialogContent extends StatefulWidget {
  ...
  @override
  State<_BasicScoreDialogContent> createState() =>
      _BasicScoreDialogContentState();
}
  ...
  Widget build(BuildContext context) {
    return StatefulBuilder(
      builder: (context, setDialogState) {
        ...
        final isScoreTooHigh =
            widget.maxScore != null &&
            enteredScore != null &&
