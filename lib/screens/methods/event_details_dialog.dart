// lib/screens/methods/event_details_dialog.dart
...
/// Shows event details dialog for the selected practice and firearm
Future<void> showEventDetailsDialog({
  required BuildContext context,
  required String? practiceName,
  required String? firearmCode,
}) async {
  // Validate inputs
  ...
  try {
    ...
    if (firearmId == null) {
      return; // Don't show dialog if firearm ID not found
    }

    // Verify firearm is in applicable list
    if (!matchedEvent.applicableFirearmIds.contains(firearmId)) {
      return; // Don't show dialog if firearm not applicable
    }
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Event ${event.eventNumber} • ${firearmInfo.code} (${firearmInfo.gunType})',
                          style: TextStyle(
                            fontSize: 13,
